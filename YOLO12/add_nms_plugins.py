...
import argparse
def create_attrs(topK, keepTopK, num_class):
    attrs = {}
    attrs["shareLocation"] = 1 # Default for yolor
    attrs["backgroundLabelId"] = -1
    attrs["numClasses"] = num_class
    attrs["topK"] = topK
    attrs["keepTopK"] = keepTopK
    attrs["scoreThreshold"] = 0.2
...
def create_and_add_plugin_node(graph, topK, keepTopK, num_class):
    ...
    mns_node = gs.Node(
        ...
        attrs=create_attrs(topK, keepTopK, num_class),
...
def main():
    ...
    parser.add_argument("-f", "--model", help="Path to the ONNX model generated by export_model.py", default="./weights/last.onnx")
    ...
    parser.add_argument("-m", "--num_class", help="number of class", default=7)

    args, _ = parser.parse_known_args()
    print("Number of classes:", int(args.num_class))
    graph = gs.import_onnx(onnx.load(args.model))
    
    graph = create_and_add_plugin_node(graph, int(args.topK), int(args.keepTopK), int(args.num_class))
    
    onnx.save(gs.export_onnx(graph), args.model.replace('.onnx', '') + "_nms.onnx")
